...
CREATE TABLE IF NOT EXISTS `members` (
  ...
  `is_former_staff_member` BIT(1) NOT NULL DEFAULT 0,
...
-- -----------------------------------------------------
-- Placeholder table for view `non_deleted_parties`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `non_deleted_parties` (`party_id` INT, `name` INT, `date` INT, `normal_beer_price` INT, `special_beer_price` INT);

-- -----------------------------------------------------
-- Placeholder table for view `non_deleted_members`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `non_deleted_members` (`member_id` INT, `first_name` INT, `last_name` INT, `school` INT, `email` INT, `phone` INT, `balance` INT, `last_membership_payment` INT);

-- -----------------------------------------------------
-- View `non_deleted_parties`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `non_deleted_parties` ;
DROP TABLE IF EXISTS `non_deleted_parties`;
USE `sbeereck-db`;
CREATE  OR REPLACE VIEW `non_deleted_parties` AS
SELECT party_id, name, date, normal_beer_price, special_beer_price
	FROM parties
    WHERE is_deleted = 0
    ORDER BY date DESC;

-- -----------------------------------------------------
-- View `non_deleted_members`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `non_deleted_members` ;
DROP TABLE IF EXISTS `non_deleted_members`;
USE `sbeereck-db`;
CREATE  OR REPLACE VIEW `non_deleted_members` AS
SELECT member_id, first_name, last_name, school, email, phone, balance, last_membership_payment
	FROM members
    WHERE is_deleted = 0
