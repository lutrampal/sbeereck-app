...
SELECT p.party_id, name, date, number_of_attendees, balance
	FROM non_deleted_parties AS p, (SELECT party_id, COUNT(DISTINCT member_id) number_of_attendees, SUM(amount) balance 
										FROM transactions 
										GROUP BY party_id) AS d
	WHERE p.party_id = d.party_id
		AND is_deleted = 0
	ORDER BY date DESC;
